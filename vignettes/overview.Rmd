...
If gene expression data are not provided, no comparision for gene expression will be made for the differentially methylated groups. 
...
The Regular mode targets DNA methylation analysis on cis-regulatory elements within or immediately surrounding protein-coding genes 
...
# Since enhancers are cell or tissue-type specific, EpiMix needs to know the reference cell or tissue type(s) in order to select the proper enhancers.Available epigenome groups (a group of relevant cell types) or epigenome ids (individual cell types) can  be obtained from **Figure 2** of the [RoadmapEpigenomic publication](https://www.nature.com/articles/nature14248). Alternatively, they can also be retrieved from the EpiMix's built-in function `list.epigenomes()`.
...
EpiMix enables automated DNA methylation analysis for cancer data from the TCGA project with a single-function `TCGA_GetData`. This function wraps the functions for (1) data downloading, (2) pre-processing and (3) DNA methylation analysis.
...
The above one-step functions can be executed in a step-by-step manner in case users want to inspect the output from each individual step. 
...
### 7.1 Mixture model of the DNA methylation, violin plot of the gene expression, and correlation plot of the DNA methylation and gene expression
...
```{r, message = FALSE, warning=FALSE, fig.show="hold", dev='png', out.width="110%", results = 'hide', fig.cap = "Integrative visualization of the chromatin state, DM values and the transcript structure of the *CCND2* gene. The differential methylation (DM) value represents the mean difference in beta values between the hypermethylated samples versus the normally methylated samples"}
library(EpiMix)
library(karyoploteR)
...
```{r, message = FALSE, warning=FALSE, fig.show="hold", dev='png', out.width="110%", results = 'hide', fig.cap = "Integrative visualization of the chromatin state and the nearby genes of a differentially methylated CpG site"}
library(EpiMix)
library(karyoploteR)
library(TxDb.Hsapiens.UCSC.hg19.knownGene)
library(org.Hs.eg.db)
...
knitr::kable(enrich.results@result[1:3,], row.names = FALSE)
```
...
knitr::kable(enrich.results@result[1:5,], row.names = FALSE)
```
<br>
...
survival.CpGs <- get.survival.probe (EpiMixResults = Sample_EpiMixResults_miRNA, TCGA_CancerSite = CancerSite)
