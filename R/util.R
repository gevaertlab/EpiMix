...
get.survival.probe <- function(EpiMixResults,
  ...
  survival.results <- data.frame(Probe = Probe, State = State, HR = hazard.ratio, lower.cl = low.conf, higher.cl = high.conf,p.value = pval)
  ...
  if(OutputRoot!="" & length(survival.results) > 0){
    cat(paste0("Saving the result to ", OutputRoot, "/", "Survival_Probes.csv"))
    utils :: write.csv(survival.results, paste0(OutputRoot, "/", "Survival_Probes.csv"))
  }
