###################################################################################################################
#                                  Helper functions used by EpiMix
###################################################################################################################
...
filterProbes <- function(mode, gene.expression.data, listOfGenes, promoters, met.platform, genome){
  cat("Fetching probe annotation...\n")
  ProbeAnnotation = getProbeAnnotation(mode = mode, met.platform = met.platform, genome = genome)
  if(!is.null(listOfGenes)){
    warning("Please input the selected gene names in the upper case format: e.g., IGF1,  CCND3, MIRLET7A1, MIR10226, LINC01409, TTLL10-AS1", immediate. = TRUE)
    if(mode == "miRNA"){
  ...
  if(!is.null(gene.expression.data)){
    # we only select the CpGs associated with genes with gene expression data available
...
generateFunctionalPairs <- function(MET_matrix, MET_Control, gene.expression.data, ProbeAnnotation, raw.pvalue.threshold, adjusted.pvalue.threshold, cores){
  ...
  if(cores == "" | cores == 1){
    for(i in 1:iterations){
      ...
      pairs =  getFunctionalProbes(gene, probes, MET_matrix, gene.expression.data, raw.pvalue.threshold = raw.pvalue.threshold, adjusted.pvalue.threshold = adjusted.pvalue.threshold)
    ...
    FunctionalPairs <- foreach :: foreach(i = 1:iterations, .combine = rbind, .options.snow= opts, .verbose = FALSE)  %dopar% {
      gene = uniqueGenes[i]
      probes = ProbeAnnotation$probe[which(ProbeAnnotation$gene == gene)]
      getFunctionalProbes(gene, probes, MET_matrix, gene.expression.data, raw.pvalue.threshold = raw.pvalue.threshold, adjusted.pvalue.threshold = adjusted.pvalue.threshold)
    }
  }
  close(pb)
