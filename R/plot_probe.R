...
#' @param probe.name character string indicating the probe name.
#' @param EpiMixResults  list object returned from EpiMix
#' @param met.platform  character string indicating the type of Illumina Infinium DNA methylation BeadChip for collecting the DNA methylation data.Can be either "HM450" or "EPIC". Default: "HM450"
#' @param numFlankingGenes numeric value indicating the number of flanking genes to be plotted with the probe. Default: 20 (10 gene upstream and 10 gene downstream).
...
#' @param roadmap.epigenome.id string indicating the epigenome id (EID) for a specific tissue or cell type whose DNase-seq and histone ChIP-seq data to be retrieved. The EID can be found in the figure 2 of this paper: PMID: 25693563
...
plot.probe <- function(probe.name,
  ...
  genePosition <- getBM(filters= "hgnc_symbol",
  ...
  gene.region <- toGRanges(data.frame(chr_name, left_end,right_end))
  ...
  kp <- plotKaryotype(genome = genome, zoom = gene.region, plot.type = 2, plot.params = pp)
  kpAddBaseNumbers(kp,
  ...
  probe_pos = start(ranges(ProbeAnnotation))
  kpLines(kp,
          chr = chr_name,
  ...
  kpAddLabels(kp, data.panel = 2, labels="Probe",r0=r0_probe,r1=r1_probe,
  ...
  r1_Gene = 0.98
  kpDataBackground(kp, data.panel = 2, r0=r0_Gene - 0.03,r1=r1_Gene + 0.01, col="#AACBFF")
  ...
  kpAddLabels(kp, labels="Genes",data.panel = 2, r0=r0_Gene,r1=r1_Gene, srt=90,
              pos=1, label.margin = y.label.margin, cex = y.label.font)


# Plot histone marks and TF binding sites
  cat("Retrieving signal intensity of DNA-binding protiens from RoadmapEpigenomics...\n")
  ...
  colors <-  brewer.pal(total.tracks, "Set3")
