# Script to download DNA methylation data and gene expression data from GEO
...
GEO_getSampleMap <- function(METdirectories, GEdirectories, targetDirectory = ".") {
    ...
    sample_met <- data.frame(assay = rep("DNA methylation", nrow(pData.met)), primary = pData.met$title,
        colnames = pData.met$geo_accession)
    cat("Found", nrow(pData.met), "samples in DNA methyatlion data.\n")
    sample_exp <- data.frame(assay = rep("Gene expression", nrow(pData.exp)), primary = pData.exp$title,
        colnames = pData.exp$geo_accession)
    ...
    if (targetDirectory != "" | !is.null(targetDirectory)) {
        ...
        utils::write.csv(sample.map, paste0(targetDirectory, "/", "sample.map.csv"),
            row.names = FALSE)
        cat("Sample map has been saved to:", paste0(targetDirectory, "/", "sample.map.csv\n"))
    }
    overlapSamples <- intersect(sample.map$primary[which(sample.map$assay == "DNA methylation")],
        sample.map$primary[which(sample.map$assay == "Gene expression")])
...
GEO_Preprocess_GeneExpression <- function(gene.expression.data, sample.info = NULL,
    ...
    # Step 3: Mapping ENSG gene names to external gene names and remove
    # duplicated genes
    ...
    if (!is.null(sample.info) & !is.null(group.1) & !is.null(group.2)) {
        ...
        if (ncol(MET_Experiment) == 0)
            stop("Cannot find methylation data with samples in the group.1 ! The sample names must overlap with the column names of the gene expression data.")
        if (ncol(MET_Control) == 0)
            stop("Cannot find methylation data with samples in the group.2 ! The sample names must overlap with the column names of the gene expression data.")
        cat("There are", ncol(MET_Experiment), "samples in the", paste0(group.1,
            collapse = " and "), "group, and", ncol(MET_Control), "samples in the",
            paste0(group.2, collapse = " and "), "group.\n")
    } else {
...
convertGeneNames <- function(gene.expression.data) {
    ...
    ensembl_gene_map <- biomaRt::getBM(filters = filter, attributes = c(filter, "hgnc_symbol",
        "chromosome_name"), values = ensemblID, mart = mart)
    ...
    # get rid of those genes on the haplotypic region (chromosome names
    # starting with 'CHR')
    ensembl_gene_map <- ensembl_gene_map[!startsWith(ensembl_gene_map$chromosome_name,
        "CHR"), ]  #16,033
    ...
    # cat('Removing', nrow(gene.expression.data) - length(overlapGenes),
    # 'transcripts that can not be mapped to human genes.\n')
...
#' @keywords internal
#'
GEO_EstimateMissingValues_Molecular <- function(MET_Data, MissingValueThresholdGene = 0.3,
