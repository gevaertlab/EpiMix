...
#' }
#'
EpiMix_PlotProbe <- function(probe.name, EpiMixResults, met.platform = "HM450", roadmap.epigenome.id = "E002",
    numFlankingGenes = 20, left.gene.margin = 10000, right.gene.margin = 10000, gene.name.pos = 2,
    gene.name.size = 0.5, gene.arrow.length = 0.05, gene.line.width = 2, plot.chromatin.state = TRUE,
    ...
    NearbyGenes <- GetNearGenes(geneAnnot = geneAnnot, TRange = ProbeAnnotation,
        numFlankingGenes = numFlankingGenes)
    ...
    genePosition <- biomaRt::getBM(filters = "hgnc_symbol", attributes = c("hgnc_symbol",
        "chromosome_name", "start_position", "end_position", "strand"), values = NearbyGenes$Symbol,
        mart = mart)

    # if a gene is postioned on the opposite strand, flip the start and end
    # position
    ...
    kp <- karyoploteR::plotKaryotype(genome = genome, zoom = gene.region, plot.type = 2,
        plot.params = pp)
    karyoploteR::kpAddBaseNumbers(kp, tick.dist = (right_end - left_end)/6, minor.tick.dist = (right_end -
        left_end)/12, add.units = TRUE, cex = 0.5, tick.len = 3)

    # Plot the probe
    r0_probe <- 0
    r1_probe <- r0_probe + 0.2
    probe_pos <- GenomicRanges::start(ranges(ProbeAnnotation))
    karyoploteR::kpLines(kp, chr = chr_name, x = c(probe_pos, probe_pos), data.panel = 2,
        y = c(0.1, 0.9), r0 = r0_probe, r1 = r1_probe, col = "purple")

    karyoploteR::kpText(kp, chr = chr_name, x = c(probe_pos), y = c(0.5), data.panel = 2,
        r0 = r0_probe, r1 = r1_probe, col = "purple", label = names(ProbeAnnotation),
        pos = 4, cex = 0.7)


    karyoploteR::kpAddLabels(kp, data.panel = 2, labels = "Probe", r0 = r0_probe,
        r1 = r1_probe, srt = 90, pos = 1, label.margin = y.label.margin, cex = y.label.font)


    # Plot the gene position
    r0_Gene <- r1_probe + 0.04
    r1_Gene <- 0.98
    karyoploteR::kpDataBackground(kp, data.panel = 2, r0 = r0_Gene - 0.03, r1 = r1_Gene +
        0.01, col = "#AACBFF")

    # Plot the genes whose expression were not different
    if (nrow(Other_genePosition) > 0) {
        karyoploteR::kpArrows(kp, chr = chr_name, data.panel = 2, r0 = r0_Gene, r1 = r1_Gene,
            x0 = Other_genePosition$start_position, x1 = Other_genePosition$end_position,
            y0 = seq(0.2, 0.8, length.out = nrow(Other_genePosition)), y1 = seq(0.2,
                0.8, length.out = nrow(Other_genePosition)), lwd = gene.line.width,
            length = gene.arrow.length)

        karyoploteR::kpText(kp, chr = chr_name, data.panel = 2, r0 = r0_Gene, r1 = r1_Gene,
            x = Other_genePosition$start_position, y = seq(0.2, 0.8, length.out = nrow(Other_genePosition)) +
                0.05, labels = Other_genePosition$hgnc_symbol, pos = gene.name.pos,
            cex = gene.name.size)
    }
    ...
    if (nrow(DEGenes_genePosition) > 0) {
        karyoploteR::kpArrows(kp, chr = chr_name, data.panel = 2, r0 = r0_Gene, r1 = r1_Gene,
            x0 = DEGenes_genePosition$start_position, x1 = DEGenes_genePosition$end_position,
            y0 = seq(0.2, 0.7, length.out = nrow(DEGenes_genePosition)), y1 = seq(0.2,
                0.7, length.out = nrow(DEGenes_genePosition)), lwd = gene.line.width,
            col = "red", length = gene.arrow.length)


        karyoploteR::kpText(kp, chr = chr_name, data.panel = 2, r0 = r0_Gene, r1 = r1_Gene,
            x = DEGenes_genePosition$start_position, y = seq(0.2, 0.7, length.out = nrow(DEGenes_genePosition)) +
                0.05, labels = DEGenes_genePosition$hgnc_symbol, pos = gene.name.pos,
            offset = 0.2, cex = gene.name.size, col = "red")
    }
    karyoploteR::kpAddLabels(kp, labels = "Genes", data.panel = 2, r0 = r0_Gene,
        r1 = r1_Gene, srt = 90, pos = 1, label.margin = y.label.margin, cex = y.label.font)
